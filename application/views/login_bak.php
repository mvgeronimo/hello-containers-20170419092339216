	...
	$(document).on('click', '#login_btn', function(){
		...
		if(username != '' || password != '') {
			...
			$.ajax({
	          ...
	          'url' : '<?=base_url()?>azure/pwgrant.php',
	          ...
	          'type' : 'post'
            	...
            	if(obj.status=='success'){
					...
					$.ajax({
						...
						url:'<?=base_url();?>login_controller/verifyUser',
						...
						if(obj.result == '1') {
							...
							var agenda_id = "<?=(isset($_GET['id'])) ? $_GET['id'] : '' ?>";   //$('.agenda_id').val();
							...
							if(agenda_id != '') {
								...
								if(user_type == '2') {
									...
									$.ajax({
										...
										url:'<?=base_url();?>login_controller/check_if_psr',
										...
										if(result == 'not') {
											...
											bootbox.alert("<b>You are not the PSR of this agenda.</b>", function() {
												...
												window.location = '<?=base_url();?>login_controller/logout';

											})

										} else {

											window.location = '<?=base_url();?>Agenda/psr_comment/'+agenda_id;
									...
									window.location = '<?=base_url();?>Agenda/om_comment/'+agenda_id;
...
<div class="container login_container">
	...
	<input type="hidden" class="agenda_id" value="<?=(isset($_GET['id'])) ? $_GET['id'] : '' ?>">
...
<div class="col-md-6 footer-login foots2" style="text-align:right">
